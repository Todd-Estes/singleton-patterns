...
If you define a metaclass but do not explicitly inherit from type, Python will fall back to the default base class for normal classes, which is object. However, for metaclasses to work correctly, they must inherit from type.

If Singleton does not inherit from type, it won't function as a valid metaclass. Instead, it will just be a regular class inheriting from object. This is why you explicitly inherit type when creating a metaclass.
...
class_logger = Logger()
class_logger.log("Hello!")
print(type(new_logger))
